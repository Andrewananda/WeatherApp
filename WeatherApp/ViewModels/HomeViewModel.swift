...
class HomeViewModel {
	...
	func fetchCurrentWeather(lon: Double, lat: Double) {
		...
		apiService.fetchData(url: url, method: .get, params: nil) { [unowned self] (response: Result<CurrentWeatherResponse, Errors>) in
			switch response {
	...
	func fetchWeatherHistory(lon: Double, lat: Double) {
		...
		apiService.fetchData(url: url, method: .get, params: nil) { [unowned self] (response: Result<WeatherHistoryResponseNew, Errors>) in
			switch response {
			case .success(let data):
